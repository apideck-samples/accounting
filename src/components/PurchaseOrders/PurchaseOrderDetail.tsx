...
const PurchaseOrderDetail = ({ purchaseOrder, onClose }: Props) => {
  const currency = purchaseOrder?.currency?.toString() || 'USD'
  ...
  return (
    <div className="divide-y divide-gray-200 dark:divide-gray-700">
      {/* Header Section */}
      <div className="pb-6">
        <div className="h-24 bg-gradient bg-gradient-to-r from-cyan-600 to-blue-600 sm:h-20 lg:h-28" />
        <div className="-mt-12 flow-root px-4 sm:-mt-8 sm:flex sm:items-end sm:px-6 lg:-mt-20">
          <div>
            <div className="-m-1 flex">
              <div className="inline-flex overflow-hidden rounded-lg border-4 border-white bg-white dark:border-gray-800 dark:bg-gray-700">
                <HiOutlineOfficeBuilding className="h-24 w-24 p-4 text-gray-500 dark:text-gray-400 sm:h-40 sm:w-40 lg:h-44 lg:w-44 p-8" />
              </div>
            </div>
          </div>
            ...
            <div className="sm:flex sm:min-w-0 sm:flex-1 sm:items-center sm:justify-between">
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 truncate text-sm text-gray-500 dark:text-gray-400">
                  PO: {purchaseOrder.poNumber || purchaseOrder.id}
                </p>
              </div>
              <div className="mt-4 flex flex-shrink-0 items-center gap-x-3 sm:mt-0 sm:ml-4">
                {purchaseOrder.status && (
                  <Chip
                    label={purchaseOrder.status.toString()}
                    className="inline-flex uppercase"
                    colorClassName="bg-blue-100 text-blue-800 dark:bg-blue-700 dark:text-blue-100"
                  />
                )}
                <Menu as="div" className="relative inline-block text-left">
                  <Menu.Button className="inline-flex items-center rounded-md border border-gray-300 bg-white p-2 text-sm font-medium text-gray-400 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600">
      ...
      {deleteErrorIssues && deleteErrorIssues.length > 0 && (
        <div className="px-4 pt-2 sm:px-6">
          <FormErrors issues={deleteErrorIssues} title="Could not delete purchase order:" />
        </div>
      )}
          ...
          <div className="sm:flex sm:px-6 sm:py-5">
            ...
            <dd className="mt-1 text-sm text-gray-900 dark:text-white sm:col-span-2 sm:mt-0 sm:ml-6">
              {purchaseOrder.issuedDate
                ? new Date(String(purchaseOrder.issuedDate)).toLocaleDateString()
                : 'N/A'}
            </dd>
          </div>
          <div className="sm:flex sm:px-6 sm:py-5">
            ...
            <dd className="mt-1 text-sm text-gray-900 dark:text-white sm:col-span-2 sm:mt-0 sm:ml-6">
              {purchaseOrder.deliveryDate
                ? new Date(String(purchaseOrder.deliveryDate)).toLocaleDateString()
                : 'N/A'}
            </dd>
          </div>
          <div className="sm:flex sm:px-6 sm:py-5">
            ...
            <dd className="mt-1 text-sm text-gray-900 dark:text-white sm:col-span-2 sm:mt-0 sm:ml-6">
              {purchaseOrder.total !== undefined &&
                ...
                new Intl.NumberFormat(currency, { style: 'currency', currency: currency }).format(
                  purchaseOrder.total
                )}
            </dd>
          </div>
                      ...
                      <div className="overflow-hidden shadow ring-1 ring-black dark:ring-gray-700 ring-opacity-5 md:rounded-lg">
                        <table className="min-w-full divide-y divide-gray-300 dark:divide-gray-600">
                          ...
                          <tbody className="bg-white dark:bg-gray-900">
                            {purchaseOrder.lineItems?.map((lineItem, i: number) => (
                              <tr
                              ...
                              >
                                <td className="py-4 pl-4 pr-3 text-sm sm:pl-6">
                                  ...
                                  <div className="mt-1 text-gray-500 dark:text-gray-400 truncate max-w-xs">
