...
import type { BankAccount, Supplier } from '@apideck/unify/models/components' // Ensure all necessary types are imported
import { Menu, Transition } from '@headlessui/react'
import { Fragment, useState } from 'react'

import Spinner from 'components/Spinner'
import { useSuppliers } from 'hooks' // Assuming useSuppliers hook is created and provides deleteSupplier
import {
  HiDotsVertical,
  HiOutlineCreditCard,
  HiOutlineGlobeAlt,
...
const SupplierDetails = ({ supplier, onClose }: Props) => {
  ...
  const onDelete = async (id: string) => {
    ...
    try {
      ...
      if (result) {
        // deleteSupplier in hook shows its own success/error toasts and returns null on fetch error
        addToast({
          title: 'Supplier deleted',
          description: `Supplier ${supplier.displayName || supplier.id} has been deleted.`,
          type: 'success'
        })
        onClose()
      } else if (!result) {
        // Toast for deletion failure is handled by the hook, just ensure loading stops
      }
    } catch (error: any) {
      ...
      addToast({
        ...
        description:
          error?.message || 'An unexpected error occurred while trying to delete the supplier.',
        type: 'error'
      })
    }
    setIsLoading(false)
  ...
  return (
    <div className="divide-y divide-gray-200 dark:divide-gray-700">
      {/* Header Section */}
      <div className="pb-6">
        <div className="h-24 bg-gradient-to-r from-purple-600 to-indigo-600 sm:h-20 lg:h-28 rounded-md" />
        <div className="-mt-12 flow-root px-4 sm:-mt-16 sm:flex sm:items-end sm:px-6 lg:-mt-20">
          <div>
            <div className="-m-1 flex">
              <div className="inline-flex overflow-hidden rounded-lg border-4 border-white dark:border-gray-800 bg-white dark:bg-gray-700">
                {/* Placeholder Icon, replace with actual image if available */}
          ...
          <div className="mt-6 sm:ml-6 sm:flex-1">
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">ID: {supplier.id}</p>
              {supplier.status && (
                <Chip
                  label={supplier.status.toString()}
                  className={`mt-2 uppercase inline-flex`}
                  colorClassName={
            ...
            <div className="mt-5 flex flex-wrap space-y-3 sm:space-y-0 sm:space-x-3">
              ...
              <div className="ml-auto inline-flex sm:ml-0">
                <Menu as="div" className="relative inline-block text-left">
                  ...
                  >
                    <Menu.Items className="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white dark:bg-gray-700 shadow-lg ring-1 ring-black ring-opacity-5 dark:ring-gray-600 focus:outline-none">
                      <div className="py-1">
                        {/* <Menu.Item>
                          {({ active }) => (
                            <a href="#" className={`${active ? 'bg-gray-100 dark:bg-gray-600 text-gray-900 dark:text-white' : 'text-gray-700 dark:text-gray-200'} block px-4 py-2 text-sm`}>
                              Edit Supplier (Not Implemented)
                            </a>
                          )}
                        </Menu.Item> */}
                        <Menu.Item>
        ...
        <dl className="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2">
          {supplier.companyName && (
            <div className="sm:col-span-1">
              ...
              <dd className="mt-1 text-sm text-gray-900 dark:text-white">{supplier.companyName}</dd>
            </div>
          )}
